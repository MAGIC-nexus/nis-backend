...
#
# 1) Build:
# docker build -t magic-nis-backend .
#
# Usage example:
# 2) Create (using a configuration file)
# docker create --name nis-local -p 8080:80 -e MAGIC_NIS_SERVICE_CONFIG_FILE="nis_docker_naples.conf" magic-nis-backend:latest
# 2) Create (using autogenerated configuration)
# docker create --name nis-local -p 8080:80 magic-nis-backend:latest
# 2) Create, run and remove on exit, using autogenerated configuration (NO NEED FOR STEPS 3, 4)
# docker run --rm -p 8085:80 magic-nis-backend:latest
#
...
RUN pip3 install --no-cache-dir --upgrade pip && \
    pip3 install --no-cache-dir git+https://github.com/Supervisor/supervisor gunicorn psycopg2==2.7.3.2
