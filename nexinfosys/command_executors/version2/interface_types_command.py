...
class InterfaceTypesCommand(BasicCommand):
    ...
    def _process_row(self, field_values: Dict[str, Any]) -> None:
        ...
        if ft_parent:
            ...
            if len(parent) > 0:
                ...
                if not isinstance(parent, FactorType):
                    self._add_issue(IType.ERROR, f"Parent interface type name '{ft_parent}' not found in hierarchy '{ft_h_name}")
                    return
            else:
                self._add_issue(IType.ERROR, f"Parent interface type name '{ft_parent}' not found")
                return
            # Double check, it must be defined in "hie"
            if ft_parent not in hie.codes:
                self._add_issue(IType.ERROR, f"Parent interface type name '{ft_parent}' not registered in the hierarchy '{ft_h_name}'")
                return
        else:
            parent = None
        ...
        if len(ft) == 0:
            ...
            self._add_issue(IType.ERROR, f"Interface type name '{ft_name}' already registered")
            return
